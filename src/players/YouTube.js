...
export default class YouTube extends Component {
  ...
  load(url, isReady) {
    ...
    if (isReady) {
      if (
        MATCH_PLAYLIST.test(url) ||
        MATCH_USER_UPLOADS.test(url) ||
      ...
      this.player.cueVideoById({
        videoId: id,
        index: parseIndexVideo(url) | playerVars.index,
        startSeconds: parseStartTime(url) || playerVars.start,
        endSeconds: parseEndTime(url) || playerVars.end,
      });
      return;
    }
    getSDK(SDK_URL, SDK_GLOBAL, SDK_GLOBAL_READY, (YT) => YT.loaded).then(
      (YT) => {
  ...
  onStateChange = (event) => {
    ...
    const {
      ...
      onReady,
      loop,
      config: { playerVars, onUnstarted },
    } = this.props;
    const { UNSTARTED, PLAYING, PAUSED, BUFFERING, ENDED, CUED } = window[
      SDK_GLOBAL
    ].PlayerState;
