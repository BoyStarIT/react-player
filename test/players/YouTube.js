...
test("load() when ready", (t) => {
  ...
  t.true(
    instance.player.cueVideoById.calledOnceWith({
      videoId: "oUFJJNQGwhk",
      index: 0,
...
test("onStateChange() - pause", async (t) => {
  ...
  const instance = shallow(
    <YouTube url={TEST_URL} onPause={onPause} config={{}} />
  ).instance();
...
test("onStateChange() - buffer", async (t) => {
  ...
  const instance = shallow(
    <YouTube url={TEST_URL} onBuffer={onBuffer} config={{}} />
  ).instance();
...
test("onStateChange() - ended", async (t) => {
  ...
  const instance = shallow(
    <YouTube url={TEST_URL} onEnded={onEnded} config={{}} />
  ).instance();
...
test("onStateChange() - ready", async (t) => {
  ...
  const instance = shallow(
    <YouTube url={TEST_URL} onReady={onReady} config={{}} />
  ).instance();
